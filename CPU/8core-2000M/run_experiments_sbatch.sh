...
config=/root/codes/Victima/sniper/config/UniNDP/CPU/baseline_CPU_8core.cfg
...
run_and_notify() {
    ...
    (cd "$dir" && sbatch -n 4 --partition high_latency \
    --nodelist kratos0,kratos5,kratos6,kratos10 -J CPU_8core_$dir \
    /home/qjiang/codes/Victima/docker_wrapper.sh \
    "podman run --rm -v /home/qjiang/:/root/ victima $sniper \
    -s stop-by-icount:$num_instructions --genstats --power -d ${PWD//\/home\/qjiang\//\/root\/} -c ${config} \
